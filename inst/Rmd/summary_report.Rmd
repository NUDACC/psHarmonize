---
title: "Harmonized summary Report"
...
cont_one_visit <- function(){
  ...
  output <- knitr::knit_child("./RMarkdown_child/cont_one_visit.Rmd", quiet = TRUE)
...
cont_multi_visit <- function(){
  ...
  output <- knitr::knit_child("./RMarkdown_child/cont_multi_visit.Rmd", quiet = TRUE)
...
cat_multi_visit <- function(){
  ...
  output <- knitr::knit_child("./RMarkdown_child/cat_multi_visit.Rmd", quiet = TRUE)
...
for(current_cohort in cohorts)
{
  ...
  for(current_cat in categories)
  {
    ...
    } else if(is.character(long_dataset[[current_cat]]))
    {
...
for(current_cat in categories)
{
  ...
  num_visits <- long_dataset %>%
    filter(cohort %in% cohorts & !is.na(eval(parse(text = current_cat)))) %>%
    summarise(num_visits = n_distinct(visit)) %>%
    pull(num_visits)
  ...
  if('final_units' %in% names(harmonization_sheet))
  {
    ...
    units <- harmonization_sheet %>%
      filter(study %in% cohorts & item == current_cat) %>%
      filter(!is.na(final_units) & !(final_units == '') & !(final_units == ' ')) %>%
      distinct(final_units) %>%
      pull(final_units)
  ...
  data_intermediate <- long_dataset %>%
    ...
    rename(value = {{ current_cat }}) %>%
    #concatenate cohort to visit for display
    mutate(visit=paste0(cohort,'::',visit))
  ...
  if(num_unique_val > 10 & num_visits > 1 & is.numeric(long_dataset[[current_cat]]))
  {
    ...
    # Identify which visit
    visit_to_use <- data_intermediate %>%
      filter(cohort %in% cohorts & !is.na(value)) %>%
      distinct(visit) %>%
      pull(visit)
